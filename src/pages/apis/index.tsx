...
import { getApis, deleteApiById } from 'apiSdk/apis';
import { ApiInterface } from 'interfaces/api';

type ColumnType = ColumnDef<ApiInterface, unknown>;
...
export function ApiListPage(props: ApiListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ApiInterface>>(
    () => `/apis?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ApiInterface) => {
    if (hasAccess('api', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/apis/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/apis/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('api', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/apis/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('api', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="api" />
        </Flex>

        {hasAccess('api', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/apis/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
