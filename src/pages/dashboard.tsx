...
import { PropertyListPage as ShowCaseList } from 'pages/properties';
...
function DashboardPage() {
  ...
  const datasetStat = useSWR<PaginatedInterface<DatasetInterface>>(
    () => `/datasets`,
    () => getDatasets({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Datasets',
      result: datasetStat?.data?.totalCount,
      isLoading: datasetStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Properties',
